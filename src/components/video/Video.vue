      ...
      <van-swipe :show-indicators="false" @change="onChange" vertical :loop="false">
        <van-swipe-item v-for="(item, index) in videoList" :key="index" class="product_swiper">
          <div class="video_container">
            ...
            <!-- 播放暂停按钮 -->
            <div v-show="iconPlayShow" class="icon_play" @click="playvideo">
          ...
          <div class="production_box">
            <div class="production_name">@superKM</div>
            <div
  ...
  methods: {
    ...
    getComment() {
      ...
      setTimeout(() => {
        ...
        const to_comment_id = this.to_comment_id
        if (to_comment_id !== 0 || to_comment_id !== '0') {
          // 从评论列表进来回复
    ...
    getCommentDetail(to_comment_id) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const firstComment = this.videoComment.splice(index, 1)
    ...
    checkComment() {
      if (this.comment_text === '') {
        ...
        let p_id = ''
        let p_user_id = ''
        const content = this.comment_text
    ...
    sendComment(comment_id, p_id, p_user_id, content) {
      ...
      setTimeout(() => {
        ...
        if (this.replayUserData === '') {
          ...
          const index = this.replayUserData.index
          const index2 = this.replayUserData.index2
    ...
    commentLove(item, index, index2) {
      ...
      setTimeout(() => {
        if (index2 === -1) {
          ...
          // 点赞二级评论
          this.videoComment[index].child_comment[index2].love_comment = !this.videoComment[index].child_comment[index2]
            .love_comment
          if (love_flag === 1) {
